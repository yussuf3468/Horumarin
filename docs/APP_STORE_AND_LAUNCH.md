# MIDEEYE — App Store Optimization (ASO)

## APP STORE (iOS) LISTING

### App Name (30 chars max)

`MIDEEYE — Somali Knowledge`

### Subtitle (30 chars max)

`Weydii. Wadaag. Horumar.`

### Short Description (170 chars)

Ask questions, share answers, and grow with the Somali community worldwide.
Join thousands building knowledge together on MIDEEYE.

### Full Description (4,000 chars max)

---

**MIDEEYE — The Somali Knowledge Community**

Weydii. Wadaag. Horumar.
_Ask. Share. Grow._

MIDEEYE connects the global Somali community through the power of shared knowledge.
Whether you're a student, professional, entrepreneur, or community elder —
your knowledge matters here.

**Why MIDEEYE?**

🧠 **Ask Anything**
No question is too small. Post questions on any topic and get answers from
knowledgeable community members.

💬 **Real Answers**
Quality community-voted answers. The best answers rise to the top.

🌍 **Your Community, Worldwide**
Connect with the Somali diaspora and homeland. Share expertise across continents.

📚 **Knowledge That Lasts**
Every answer becomes part of a growing public knowledge base for future generations.

🏆 **Showcase Your Work**
Highlight projects, businesses, and achievements to inspire and connect.

🔔 **Stay Informed**
Get notified when your questions are answered or your content gets attention.

**Topics We Cover**
Education • Technology • Business • Health • Culture • Law • Politics •
Engineering • Agriculture • Arts • History • Religion • Language

**Languages**
Somali • English • Arabic

**MIDEEYE is free.**
Sign up in seconds. No subscription required.

---

### Keywords (100 chars — comma separated)

Somali,Somalia,African,community,Q&A,knowledge,questions,answers,education,forum,diaspora,Somalis

### Category

Primary: **Social Networking**
Secondary: **Education**

### Age Rating

**12+** (Social interaction, mild language possible from UGC)

---

## GOOGLE PLAY LISTING

### Title (50 chars max)

`MIDEEYE — Somali Knowledge Community`

### Short Description (80 chars)

`Ask questions & share knowledge with the global Somali community.`

### Full Description (4,000 chars)

---

**MIDEEYE** is the social knowledge platform built for the global Somali community.

Ask questions. Share answers. Build a better future together.

**FEATURES**

✅ **Q&A Platform** — Post questions on any topic and get real answers from the community
✅ **Knowledge Feed** — Browse questions and answers across all topics
✅ **Showcase** — Share projects, businesses, and achievements
✅ **Topics** — Organized knowledge across education, tech, business, health, culture, and more
✅ **People** — Follow experts and community members
✅ **Chat** — Connect directly with other members
✅ **Voting** — Community-powered quality signals
✅ **Notifications** — Real-time alerts for answers and interactions
✅ **Dark Mode** — Full dark theme support
✅ **Offline Support** — Browse cached content without internet

**DESIGNED FOR THE SOMALI COMMUNITY**
Whether you're in Mogadishu, Minneapolis, London, or Dubai — MIDEEYE is your home
for Somali knowledge exchange.

**SAFE & MODERATED**
Our community guidelines and moderation team ensure a respectful environment.

**Free forever. No ads. No subscription.**

---

### Category

**Social**

### Content Rating

**Teen** (Social Interaction)

### Tags

somali, somalia, knowledge, Q&A, community, education, social network, african

---

## SCREENSHOT COPY (Mobile — Carousel)

| Screen | Headline                      | Subtext                                            |
| ------ | ----------------------------- | -------------------------------------------------- |
| 1      | "Your Questions, Answered"    | Ask anything. Get real answers from the community. |
| 2      | "Somali Knowledge, Worldwide" | Connect with experts across the global diaspora.   |
| 3      | "Topics You Care About"       | Education, tech, business, culture, and more.      |
| 4      | "Showcase Your Work"          | Share projects and achievements that inspire.      |
| 5      | "Stay Connected"              | Real-time notifications and direct messaging.      |

---

## FEATURE HIGHLIGHT BULLETS

- Ask questions in Somali, English, or Arabic
- Community-voted answers surfaced to the top
- Follow experts and build your knowledge network
- Share and discover projects in the Showcase
- Browse 20+ topic categories
- Fully offline-capable Progressive Web App
- Dark mode designed for mobile-first use
- GDPR compliant. Your data is yours.

---

## LAUNCH CHECKLIST

### Pre-submission

- [ ] App icons in all required sizes (see docs/ICON_SIZES.md)
- [ ] 5–10 screenshots per device type (iPhone, iPad, Android, tablet)
- [ ] App preview video (15–30 seconds, optional but recommended)
- [ ] Privacy Policy URL: https://mideeye.com/privacy
- [ ] Support URL: https://mideeye.com/support
- [ ] App signing keystore secured and backed up
- [ ] Bundle ID: com.mideeye.app
- [ ] Version 1.0.0 (build 1)

### iOS (App Store Connect)

- [ ] Apple Developer Program enrolled ($99/year)
- [ ] Bundle identifier registered
- [ ] Distribution certificate + provisioning profile generated
- [ ] App Review Guidelines compliance reviewed
- [ ] IDFA / tracking compliance declared
- [ ] App Privacy labels filled in
- [ ] TestFlight beta review passed
- [ ] App Review notes added (explain Somali language content)

### Android (Google Play Console)

- [ ] Google Play Developer account ($25 one-time)
- [ ] Release keystore generated and backed up (NEVER lose this)
- [ ] Signed AAB (Android App Bundle) uploaded
- [ ] Data safety section completed
- [ ] Target API level ≥ 34 (Android 14)
- [ ] Internal testing → Closed testing → Production rollout plan
- [ ] Store listing assets: feature graphic (1024×500), icon (512×512)

---

## BRAND POSITIONING

**Tagline:** Weydii. Wadaag. Horumar.
**English translation:** Ask. Share. Grow.

**One-liner:** The knowledge community for the global Somali world.

**Elevator pitch:**
MIDEEYE is the first dedicated Q&A and knowledge platform for Somali speakers worldwide —
where every question contributes to a shared public library of knowledge that serves
this generation and those that follow.

---

## SOCIAL MEDIA BIOS

**Instagram** (150 chars):
🧠 Weydii. Wadaag. Horumar.
The Somali knowledge community.
Ask questions. Get answers. Grow together.
🔗 mideeye.com

**X / Twitter** (160 chars):
The Somali knowledge community. Ask, share, and grow together. Weydii. Wadaag. Horumar. 🌍

**LinkedIn** (company tagline):
MIDEEYE — Building the knowledge infrastructure for the global Somali community.
A social Q&A platform connecting learners, experts, and innovators.

**TikTok**:
🧠 Somali knowledge platform
✍️ Ask & share what you know
🌍 Global Somali community
weydii • wadaag • horumar

---

## BRAND VOICE GUIDELINES

| Dimension        | Description                                               |
| ---------------- | --------------------------------------------------------- |
| **Tone**         | Warm, intelligent, aspirational — never corporate or cold |
| **Language**     | Somali-first, English-accessible, Arabic-friendly         |
| **Personality**  | Community elder meets modern tech platform                |
| **Avoid**        | Buzzwords, excessive English jargon, condescension        |
| **Core message** | Every voice matters. Every question deserves an answer.   |

---

## 30-DAY SOCIAL CONTENT PLAN

| Day | Platform  | Content Type        | Topic                              |
| --- | --------- | ------------------- | ---------------------------------- |
| 1   | All       | Launch announcement | "MIDEEYE is live"                  |
| 2   | IG/TikTok | Reel                | "Why we built MIDEEYE"             |
| 3   | X/Twitter | Thread              | Top 5 questions this week          |
| 4   | LinkedIn  | Article             | Somali knowledge economy           |
| 5   | IG        | Carousel            | How to use MIDEEYE                 |
| 6   | TikTok    | Video               | Demo: ask your first question      |
| 7   | All       | Community spotlight | First 100 users                    |
| 8   | X         | Poll                | Most-wanted topics                 |
| 9   | IG        | Reel                | Showcase feature highlight         |
| 10  | LinkedIn  | Post                | Diaspora knowledge story           |
| 11  | TikTok    | Duet/Collab         | With community leader              |
| 12  | All       | Milestone post      | First 500 questions                |
| 13  | X         | Thread              | Behind the build                   |
| 14  | IG        | Story               | Q&A with founders                  |
| 15  | All       | 2-week review       | What the community is asking       |
| 16  | LinkedIn  | Article             | Education + technology in Somalia  |
| 17  | TikTok    | Video               | Top answers this week              |
| 18  | IG        | Carousel            | Community guidelines explainer     |
| 19  | X         | Thread              | Feature roadmap                    |
| 20  | All       | User story          | Testimonial post                   |
| 21  | TikTok    | Video               | New feature demo                   |
| 22  | LinkedIn  | Post                | Impact story                       |
| 23  | X/IG      | Infographic         | Community stats                    |
| 24  | All       | Engagement          | "Ask us anything"                  |
| 25  | TikTok    | Video               | Showcase project highlight         |
| 26  | IG        | Reel                | Language & culture topic spotlight |
| 27  | LinkedIn  | Article             | Community knowledge economy        |
| 28  | X         | Thread              | Month recap                        |
| 29  | All       | Gratitude post      | Thank you to early users           |
| 30  | All       | Growth post         | "What's next for MIDEEYE"          |

---

## EMAIL TEMPLATES

### Welcome Email (Day 0)

**Subject:** Mideeye Ku Soo Dhawow! Welcome to MIDEEYE 🧠

---

Hi [Name],

You've joined MIDEEYE — the Somali knowledge community.

Here's how to get started in 3 steps:

1. **Complete your profile** → Add a photo and bio so others know you
2. **Ask your first question** → Any topic, any language
3. **Follow 5 people** → Build your knowledge network

Your community is waiting.

[Get Started →](https://mideeye.com/dashboard)

Nabad,
The MIDEEYE Team

---

### Day 3 Re-engagement Email

**Subject:** Your question is waiting to be answered 💬

---

Hi [Name],

Have something you've been wondering about?

The MIDEEYE community is active and ready to help. Ask your first question today.

[Ask Now →](https://mideeye.com/ask)

---

### Day 7 Notification Email

**Subject:** This week on MIDEEYE 🔥

---

Hi [Name],

Here's what's trending in your topics this week:

• [Question 1 title] — [X answers]
• [Question 2 title] — [X answers]
• [Question 3 title] — [X answers]

[See All →](https://mideeye.com/questions)

To manage notifications: [Settings](https://mideeye.com/settings)

---

### Answer Notification (Transactional)

**Subject:** Someone answered your question on MIDEEYE

---

Hi [Name],

[Answerer Name] answered your question:

"[Your question title]"

[Read the Answer →](https://mideeye.com/questions/[id])

---

## EMAIL INFRASTRUCTURE NOTES

**Recommended providers:**

- **Resend** (developer-friendly, generous free tier, built-in React Email)
- **Postmark** (best transactional deliverability)
- **Amazon SES** (lowest cost at scale)

**Anti-spam compliance:**

- Include physical address in all marketing emails
- One-click unsubscribe in all marketing/notification emails (CAN-SPAM / GDPR Art. 7)
- Double opt-in for newsletter/marketing list
- Honor unsubscribe within 10 business days (aim for immediate)
- Never email users who haven't verified their address

**Segmentation:**
| Segment | Emails |
|---------|--------|
| New (0–7 days) | Welcome sequence (3 emails) |
| Active | Weekly digest, milestone alerts |
| Dormant (14+ days no login) | Re-engagement campaign |
| Churned (30+ days) | One final re-engagement, then suppress |
